...
## maybe an apply function is best here, but...
...
inf_curves <- data.frame(Well = NA, inf_point = NA, a = NA, b = NA, "R^2" = NA, 
                         "slope(x~y)" = NA, CtY0 = NA, Eff = NA, "Eff'%'" = NA, Ri = NA)         
# the second value is the inlfection point
...
log_Data <- function(rawData = NULL,Name = "logData"){
    ...
    for (cur in 1:Par$curves){
        ...
        log_Data[cur,] <- rapply(as.list(rawData[cur,]),function(n){  # in raw data for more curves
            if(n<0){
                n <- log10(minval)  # if the value is negative, put the log of the min to avoid ERROR
            } else {
                n <- log10(n)
            }
        })
...
Init_Value <- function (log_Data = NULL){
    for (cur in 1:Par$curves){ # first loop to apply all the script to each curve, should be an apply formula but...
